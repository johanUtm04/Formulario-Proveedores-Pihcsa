 ...
 * Versión: Producción 2026 (Lógica Insert/Update + Nombres Forzados)
 */
...
if (!file_exists($base_dir)) { 
    // El @ ignora el Warning si no hay permisos, pero el if revisa si se creó
    if (!@mkdir($base_dir, 0777, true)) {
        // Si falló mkdir, revisamos si al menos la carpeta 'uploads' es escribible
        if (!is_writable(__DIR__ . "/uploads/")) {
            die("Error de Permisos: La carpeta 'uploads' no permite escritura y no tengo contraseña de sudo para arreglarlo.");
        }
    }
}
if (!file_exists($base_dir)) { 
...
// 4. Función para subir archivos con nombre forzado (Evita nombres extraños en servidor)
...
// 5. Procesar las subidas (Nombres de campo del form -> Nombres físicos en carpeta)
...
// 6. Generar el PDF del Comprobante (Aviso de Privacidad Firmado)
$nombre_pdf_privacidad = "AVISO_PRIVACIDAD_FIRMADO.pdf";
$pdf = new FPDF();
$pdf->AddPage();
$pdf->SetFont('Arial', 'B', 16);
$pdf->Cell(0, 15, utf8_decode('PIHCSA PARA HOSPITALES - REGISTRO DE PROVEEDORES'), 0, 1, 'C');
$pdf->Ln(10);
$pdf->SetFont('Arial', '', 12);
$pdf->MultiCell(0, 8, utf8_decode("Razón Social: $razon_social\nRFC: $rfc\nFecha de Registro: " . date('d/m/Y') . "\n\nFirma Digital Autógrafa:\n$firma"));
$pdf->Output('F', $base_dir . $nombre_pdf_privacidad);

// 7. LÓGICA DE BASE DE DATOS (INSERT O UPDATE)
$checkRFC = "SELECT id FROM registro_proveedores WHERE rfc = '$rfc' LIMIT 1";
$resCheck = mysqli_query($conexion, $checkRFC);
$existe = (mysqli_num_rows($resCheck) > 0);

if ($existe) {
    // MODO UPDATE: Actualizamos datos de texto y archivos nuevos
    $query = "UPDATE registro_proveedores SET 
        ...
        email = '$email', 
        firma_digital = '$firma',
        archivo_privacidad = '$nombre_pdf_privacidad'";
    ...
    if($p_const) $query .= ", doc_constancia_situacion_fiscal = '$p_const'";
    if($p_opin)  $query .= ", doc_opinion_cumplimiento = '$p_opin'";
    if($p_banc)  $query .= ", doc_caratula_bancaria = '$p_banc'";
    if($p_dom)   $query .= ", doc_comprobante_domicilio = '$p_dom'";
    if($p_reg)   $query .= ", doc_registro_sanitario_vigente = '$p_reg'";
    if($p_hoja)  $query .= ", doc_hoja_seguridad_producto = '$p_hoja'";
    if($p_ine_r) $query .= ", doc_ine_representante_legal = '$p_ine_r'";
    if($p_ine_s) $query .= ", doc_ine_responsable_sanitario = '$p_ine_s'";
    if($p_placa) $query .= ", img_placa_responsable = '$p_placa'";
    if($p_fac)   $query .= ", img_fachada_calle = '$p_fac'";
    if($p_alm)   $query .= ", img_vista_interna_almacen = '$p_alm'";

    $query .= " WHERE rfc = '$rfc'";
} else {
    // MODO INSERT: Registro totalmente nuevo
    $query = "INSERT INTO registro_proveedores (
        razon_social, domicilio, poblacion, colonia, cp, estado, rfc, pagina_web, telefono, email, firma_digital, archivo_privacidad,
        doc_licencia_sanitaria, doc_aviso_responsable_sanitario, doc_aviso_funcionamiento, 
        doc_constancia_situacion_fiscal, doc_opinion_cumplimiento, doc_caratula_bancaria, 
        doc_comprobante_domicilio, doc_registro_sanitario_vigente, doc_hoja_seguridad_producto, 
        doc_ine_representante_legal, doc_ine_responsable_sanitario, 
        img_placa_responsable, img_fachada_calle, img_vista_interna_almacen
    ) VALUES (
        '$razon_social', '$domicilio', '$poblacion', '$colonia', '$cp', '$estado', '$rfc', '$web', '$telefono', '$email', '$firma', '$nombre_pdf_privacidad',
...
if (mysqli_query($conexion, $query)) {
    ...
    die("Error crítico en MariaDB: " . mysqli_error($conexion));
}
?>